...
package com.simpleserver.vmware.utils;
 ...
 *
 * 这个类包括几个“findObject”方法。你会发现
 * 类似的，在某些情况下相同的方法在FindObjects类。
 * 为什么要复制?因为这两个类有不同的接口
 * 哲学。该类具有面向对象的设计和封装
 * 连接。FindObjects类更像一个库:它是一个
 * 连接静态方法，它们使用“原始”(未封装)数据
 * 来自连接。
 *
 * 没有尝试“提出”重复的代码，特别是从
 * findObject的不同版本。这样做(或不这样做)是故意的
 * 为了便于理解代码，便于学习。
 */

/**
 * This class keeps track of the connection to vCenter.  It encapsulates the data of that connection (vimPort,
 * vimService, serviceContent, etc. and also provides methods for finding Managed Object References and Object Contents
 * from that connection.
 * This object is not thread safe. (That's an exercise for the reader. :-) <P> Example of use:<br><code>
 * VMwareConnection conn = new VMwareConnection(serverName, userName, password);<br> String clusterName =
 * WhichClusterIsMyVMIn.vm2cluster(conn, virtualMachineName);<br> System.out.printf("VM %s is in cluster %s.%n",
 * virtualMachineName, clusterName);<br> conn.close();<br> </code>
 */
public class VMwareConnection {
     ...
     * @param serverName the name or IP address of the vCenter server to connect to
     * @param userName   the user's name to login as
    ...
    /**
     * Shuts down the connection when you are done with it. If called multiple times, will only be executed once.
     */
     ...
     * @param objectType the type of the object to retrieve
    ...
    /**
     * Returns an ObjectContent which includes a Managed Object Reference (as specificed by the objectType and name
     * parameters) and properties (as specified by the list of properties). Example of use:<br> <code>ObjectContent vm =
     * conn.findObject("VirtualMachine", vmName, "runtime.host");</code><br> <code>ObjectContent hostStorageSystem =
     * conn.findObject("HostStorageSystem", hostStorageSystemMOR.getValue(),"devicePath","model");</code> <br>
     * @param objectType the type of the object to retrieve
     ...
     * @param mor        {@link ManagedObjectReference} of the entity
     * @param properties Array of properties to be looked up
     * @return ObjectContent containing the entity and all returned properties.
     * @throws RuntimeException this method converts all exceptions it gets into a RuntimeException.
     */
    ...
    /**
     * Returns a managed object in relation to a managed object passed in. Example call: <code>ManagedObjectReference
     * folder = conn.getRelatedRef( vmRef, "VirtualMachine", "parent" );</code> which returns a VM's parent folder.
     * @param objectRef    the object that "anchors" the relationship.
     * @param type         the type of the objectRef argument (not the type of the returned object)
     * @param relationship the string representing a relationship, such as "parent".
